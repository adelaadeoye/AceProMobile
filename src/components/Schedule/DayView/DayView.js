...
export default class DayView extends React.PureComponent {
  ...
  _renderLines() {
    const { format24h, start, end, navigation } = this.props;
    const offset = this.calendarHeight / (end - start)/3;
    ...
    return range(start, end + 1).map((i, index) => {
      ...
      return [
        <Text
          key={`timeLabel${i}`}
          onPress={() => navigation.navigate('Event', { event: null })}
        ...
        i === start ? null : (
          <TouchableOpacity
            onPress={() => navigation.navigate('Event', { event: null })}
            key={`line${i}`}
            style={[styles.line, { top: offset * index, width: width - 20, height: 2 }]}
          />
        ),
        // <TouchableOpacity
        //   key={`lineHalf${i}`}
        ...
        //   style={[
        //     styles.line,
        //     { top: offset * (index + 0.5), width: width - 20, height: 2 }
        //   ]}
        // />,
      ];
    });
  ...
  _renderEvents() {
    ...
    let events = packedEvents.map((event, i) => {
      ...
      function color() {
        ...
        const { LightPink, LightRed, Yellow, Violet } = Colors;
        return [LightPink, LightRed, Yellow, Violet][rand()] || 'pink';
      }
  ...
  render() {
    ...
    return (
      <TouchableOpacity activeOpacity={1}
        onPress={() => navigation.navigate('Event', { event: null })} style={{ backgroundColor: 'red' }}>
        <ScrollView
          ref={ref => (this._scrollView = ref)}
          contentContainerStyle={[
            styles.contentStyle,
            { width: this.props.width,height:(this.calendarHeight/3)+5},
          ]}
        >
